...
public class Reusable_Actions_PageObject {
    ...
    public static void selectOptionsfromdropdpwn(WebDriver driver, WebElement xpathLocator,String userValue,ExtentTest logger,String elementName){
        WebDriverWait wait = new WebDriverWait(driver,10);
        try{
            WebElement element = wait.until(ExpectedConditions.visibilityOf(xpathLocator));
            Select dropdown = new Select(element);
            dropdown.selectByVisibleText(userValue);
            logger.log(LogStatus.PASS,"Successfully select element " +userValue+ " on" +elementName);
